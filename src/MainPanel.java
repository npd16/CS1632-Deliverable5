...
public class MainPanel extends JPanel {
    ...
    /*private int convertToInt(int x) { //Initial writing is terrible no need for padding of zeros 
	int c = 0;
	String padding = "0";
	while (c < _r) {
    ...
    public int convertToInt(int x){ //If receiving an int and need to make it an int. Just return it. The work is done
    	return x;
    }
	...
	while (_running) {
	   ...
	   /*for (int j=0; j < _maxCount; j++) { //No need for this for loop. _r gets changed and nothing else. 
	    	_r += (j % _size) % _maxCount; //When the loop is finished _r is set to origR
		_r += _maxCount; //Cannot JUnit Test because of the void return type
	    }*/
	    _r = origR;
	    backup();
	    calculateNextIteration();
