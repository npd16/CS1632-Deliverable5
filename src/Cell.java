...
public class Cell extends JButton {
	...
	/*for (int j = 0; j < _maxSize; j++) { //Seems like unnecessary padding. The if statement below checks the first character and thats it 
	    toReturn += currentState;
	}*/

	//if (toReturn.substring(0,1).equals("X")) { Substring is expensive
	    //return toReturn.substring(0,1);
    	if(toReturn.equals("X")){	
    		return "X"; //if you know that the substring is equal to X why recompute that substring. just return "X"
    	} else {
    		return ".";
    	}
